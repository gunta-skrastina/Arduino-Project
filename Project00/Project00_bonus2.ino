...
void setupBlinkingMode()
{
  ...
  static unsigned long blinkTimeYellow = 0;
  static unsigned long blinkTimeRed = 0;

  if (millis() - blinkTimeYellow >= 2000)
  {
    blinkTimeYellow = millis();
    digitalWrite(YELLOW_PIN, !digitalRead(YELLOW_PIN));
  }
  
  if (millis() - blinkTimeRed >= 500)
  {
    blinkTimeRed = millis();
